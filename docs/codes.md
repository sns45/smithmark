...
| `ATTEST_SUBJECT_UNRESOLVED` | operational | `smithmark attest` could not resolve exactly one subject artifact to attest: for an mcp-server, no `--tarball` was supplied and the artifact root held zero, or more than one, `*.tgz` tarball to digest. | M2 |
