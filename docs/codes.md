...
| `DISCOVERY_FAILED` | operational | Resolving an artifact, or fetching its metadata or candidate attestations, failed operationally: an unrecognized CLI argument shape, an npm registry request that could not be sent or returned an unexpected status, an undecodable packument or attestations body, or an OCI target error other than an absent tag or referrer. The absence of attestations, an absent OCI tag, or no npm provenance found, are never this code; those are discovery notes, since verification owns `ATTESTATION_MISSING`. | M3 |
