...
2. A deeper gap the staging step cannot bridge. `--forgeseal-output`'s evidence assembly never populates a standalone signature check; it only carries SLSA, SBOM, and VEX attestations. Tested against a real, validly keyed signed forgeseal output directory: `assayward-dogfood-policy.yaml`'s `signature.required: true` denies with `SIGNATURE_REQUIRED_MISSING` regardless, because there is no evidence field this ingestion path populates that a signature check can read. The release checksums' keyless Sigstore signature (produced by `forgeseal sign` in `release.yml`) is real and independently verifiable, but is not wired into the assayward gate's decision today; doing so needs either an assayward change or a separate verification step run alongside the gate. `assayward-gate.yml`'s final step is marked `continue-on-error` for exactly this reason, so a known, documented tooling gap does not block every release, while the real decision (evaluating the dependency SBOM and provenance, which the ingestion path does support) still runs and is visible in the workflow log.
