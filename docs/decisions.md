...
Amendment 2026-07-23 (v0.2.0 keyless signing and verification implemented): the keyless deferral above is closed for the primary attestation path. `smithmark attest --keyless` obtains the ambient GitHub Actions OIDC token (a GET to `$ACTIONS_ID_TOKEN_REQUEST_URL` with `audience=sigstore` and the `$ACTIONS_ID_TOKEN_REQUEST_TOKEN` bearer header, taking the response `.value`) and drives the already implemented `signKeyless` (ephemeral key, Fulcio certificate, Rekor transparency log) against the Sigstore public good Fulcio and Rekor (overridable via `--fulcio-url`/`--rekor-url`). Outside a permitted Actions OIDC job it fails closed with `SIGNING_CONFIG_INVALID` naming the missing context; it never falls back to an interactive browser flow (CI is the target). `--key` and `--keyless` are mutually exclusive, and a non dry run requires exactly one. `smithmark verify --certificate-identity <SAN> --certificate-oidc-issuer <issuer>` now performs real keyless verification: it mirrors assayward's `nativeVerifier.Verify` sigstore-go shape (resolve a trusted root, the injected `--sigstore-trust-root` JSON for offline tests else the public good live TUF root; parse the bundle; `sgverify.NewVerifier(..., WithTransparencyLog(1), WithObserverTimestamps(1))`; verify; extract the leaf certificate) and then enforces, failing closed, that the verified certificate's SubjectAlternativeName equals the requested identity, its OIDC issuer equals the requested issuer, a Rekor inclusion was verified, and a certificate identity is present at all. The two trust modes are mutually exclusive and keyless requires both certificate flags (a half pinned identity is fail open, so it is refused). For a keyless bundle `SIGNATURE_VALID` and `REKOR_INCLUSION_VALID` now reflect the real cryptographic result. The native path is `!wasip1`; the wasip1 stub fails closed with `SIGNING_UNAVAILABLE_PLATFORM`, unchanged. What remains for the adopter re-release to confirm end to end (out of scope for offline unit tests): a real bundle signed by live Fulcio with a live Rekor inclusion verifying against the public good live TUF root and matching a real workflow identity. `NPM_PROVENANCE_VERIFIED` (verifying npm's own SLSA provenance) stays out of scope.
...
## U9: M6 Task 6.2, smithmark's own release meets the capability manifest model and forgeseal's real output shape (2026-07-17)

Task 6.2 extends `.github/workflows/release.yml` with a Phase 6 block, adds `.github/workflows/assayward-gate.yml`, and adds `assayward-dogfood-policy.yaml` at the repo root, composing forgeseal (pinned `v0.5.1`, matching the CLI surface read from source) and assayward (pinned `v0.1.0`) rather than reimplementing either. Two pieces of honest friction surfaced while wiring smithmark's own release, recorded here rather than papered over.

**The capability manifest model does not fit a CLI.** smithmark ships as a Go binary via GoReleaser, but the capability manifest model (`smithmark.yaml`, D6's `agent-capability/v1` predicate) only defines `ArtifactKind mcp-server` and `skill`. A CLI or binary kind does not exist in the v0.1 model, so "smithmark attests itself with a capability manifest" does not apply; a CLI is not an agent artifact the v0.1 model represents. Rather than inventing a fake `mcp-server` or `skill` manifest for the smithmark binary to force a fit, Task 6.2 scopes smithmark's self attestation to what genuinely applies to a binary and is real: forgeseal produces the dependency SBOM and SLSA provenance for smithmark's own release, the release binaries are signed keyless via Sigstore in CI, and the assayward gate evaluates the published release against `assayward-dogfood-policy.yaml`. A tool or binary `ArtifactKind` would be needed for a true capability style manifest of a CLI; that is out of scope for v0.1. This is the same discipline as U8's honestly recorded taxonomy gaps, applied one level up, to smithmark's own artifact kind rather than a server or skill it verifies.

**forgeseal's real output does not line up with assayward's `--forgeseal-output` adapter, confirmed by building both from source and running them against each other.** Two distinct gaps, both tested locally rather than assumed:

1. Filename mismatch. assayward v0.1.0's forgeseal adapter (`internal/forgeseal/adapter.go`) reads three hardcoded names: `slsa.sigstore-bundle.json` (or `slsa.intoto.jsonl` as a fallback), `sbom.cdx.json`, and `vex.openvex.json`. forgeseal v0.5.1's `pipeline` command names its files after the SBOM instead, `sbom.cdx.json.intoto.jsonl` and `sbom.cdx.json.intoto.jsonl.sigstore.json`, and only emits a VEX document (`vex.json`, a different name again) when `--vex-triage` runs. Run unmodified, `assayward verify --forgeseal-output <dir>` fails closed with a plain file not found error rather than silently passing; `assayward-gate.yml`'s staging step bridges this with a `cp` and a stub OpenVEX document, a tested, real fix, not an invented flag. Both gaps are filed as a cross repo work item: https://github.com/sns45/assayward/issues/2 (distinct from assayward#1, which is the Evidence widening contract). This is the third cross repo issue smithmark has filed, alongside assayward#1 and forgeseal#26.

2. A signature channel question, scoped narrowly rather than overstated. Reading assayward's engine from source, `--forgeseal-output` stages the SLSA attestation as a full Sigstore bundle envelope, and the engine runs its signature verifier over every staged attestation; the native verifier has a real keyless path that falls back to the Sigstore public good live TUF root and checks the Fulcio chain plus Rekor inclusion when no `--sigstore-trust-root` is injected. So a genuinely keyless signed SLSA bundle plausibly satisfies a signature check through this path. That is unconfirmed: the only local test used a keyed, offline signed bundle with no `--signature-ca` supplied, which fails through a different code path (`VerifyKeyedBundle`) and does not establish that the keyless path fails too. An earlier version of this note overstated the finding as an absolute "signature can never pass through `--forgeseal-output`"; that claim held only for the keyed offline path actually tested. The policy therefore sets `signature.required: false` today with the reasoning documented inline, and the gate is NOT `continue-on-error`: it genuinely blocks on the checks confirmed to work through this ingestion path (SLSA level, allowed builder, subject digest match, SBOM presence), a DENY on any of which fails the job. `signature.required` flips back to true once the first live tagged run confirms the staged keyless SLSA bundle verifies here. The release checksums' own keyless Sigstore signature (from `forgeseal sign`) is real and independently verifiable but is not fed through this gate; a `cosign verify-blob` style companion step is the future closure for that subject. The VEX check passes vacuously today because the release pipeline stages an empty statements OpenVEX stub rather than running `--vex-triage`; wiring real OSV data is future work, not a claim made here.

Both findings are recorded in full, including the exact commands and output that produced them, in the Task 6.2 report. Closing the signature gap, and confirming an actual passing enforce mode decision, is the maintainer's task on the first live tagged release run; this milestone does not claim a green run it did not produce.
