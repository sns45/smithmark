...
- **The `_meta` vendor extension mechanism (issue #356, closed).** The registry already supports reverse DNS namespaced vendor extensions under `_meta`; issue #356 settled the format of these vendor extension keys, and the live `io.modelcontextprotocol.registry/official` block every entry carries is the mechanism in use. This is what section 6.1 proposes to use for an interim, no schema change pilot of the attestation reference.
...
**Interim form (no schema change, pilot today).** Because the registry already supports reverse DNS namespaced `_meta` extensions (issue #356), a publisher can carry the same reference under a vendor namespace without any schema change:
...
- Issue #356 (closed), the reverse DNS namespaced `_meta` vendor extension key format: `https://github.com/modelcontextprotocol/registry/issues/356`
...
- OWASP Top 10 for Agentic Applications (released 2025-12-09), `ASI04` Agentic Supply Chain Vulnerabilities: `https://genai.owasp.org/2025/12/09/owasp-top-10-for-agentic-applications-the-benchmark-for-agentic-security-in-the-age-of-autonomous-ai/`
